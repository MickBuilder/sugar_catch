...
class ScanNotifier extends _$ScanNotifier {
  ...
  Future<void> scanBarcode(String barcode) async {
    ...
    try {
      ...
      print('🔍 [SCAN_PROVIDER] API response received: ${product != null ? "Product found" : "Product not found"}');

      if (product != null) {
        print('🔍 [SCAN_PROVIDER] Extracting sugar info...');
        final sugarInfo = await api.extractSugarInfo(product);
               print('🔍 [SCAN_PROVIDER] Sugar info extracted: SugarsPer100g=${sugarInfo.sugarsPer100g}, TotalSugars=${sugarInfo.totalSugarsInProduct}${sugarInfo.productUnit}, HiddenIngredients=${sugarInfo.hiddenSugarIngredients.length}');
        
        // Create the ScanState
        final newScanState = ScanState(
          product: product,
          sugarInfo: sugarInfo,
        );
        print('🔍 [SCAN_PROVIDER] Creating new ScanState: ${newScanState.product.productName}');
